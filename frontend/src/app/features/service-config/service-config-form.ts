...
function emptyProcess(): ProcessInfoDto {
  return {
    ...
    serviceUnicodeIcon: null, directAccess: 'F',
    allowTerms: 'F', allowRating: 'T',
    cardRedirectionRequired: 'F', walletRequireOtp: 'T',
    askBeforePayWallet: 'F', testFlag: 'F',
    surveyEntityId: null, surveyServiceId: null,
    actionType: null, actionUrl: null, preAuth: 'F',
    requireOtp: 'F', iosShow: 'T', androidShow: 'T', webShow: 'T',
    ...
    groupCode: null, actionUrl2: null, adminPayment: 'F',
    ...
    platformCode: null, platformIcpUserId: null, isServiceCatalogOneTime: 'F',
  };
}
...
export class ServiceConfigFormComponent implements OnInit, Dirtyable {
  ...
  /** Snapshot taken after load/save — anything different means unsaved edits. */
  ...
  private snapshotSection(key: keyof ServiceConfigPayload): void {
    ...
    base[key] = JSON.parse(JSON.stringify(this.buildPayload()[key]));
    this.savedSnapshot = JSON.stringify(base);
  }
    ...
    this.api.pkNextInfo('BPM_PROCESSES_INFO').subscribe({
      next: info => {
        ...
        if (pkInfo && !pkInfo.autoGenerated && pkInfo.nextValue != null) {
          ...
          this.takeSnapshot(); // a prefill is not a user edit
        }
      },
      error: () => {},
    });
  ...
  private prefillOrder(): void {
    this.api.nextServiceOrder().subscribe({
      next: r => {
        this.info.update(v => ({ ...v, processOrder: r.nextOrder }));
        this.takeSnapshot(); // a prefill is not a user edit
      },
      error: () => {},
    });
  ...
  saveSteps(): void {
    ...
    this.api.saveServiceSteps(this.info().processCode, this.steps()).subscribe({
      next: () => { this.saving.set(false); this.notify.success('Steps saved'); this.snapshotSection('steps'); },
      error: err => { this.saving.set(false); this.notify.error(err, 'Save failed'); },
    });
  }
  ...
  saveFees(): void {
    ...
    this.api.saveServiceFees(this.info().processCode, this.fees()).subscribe({
      next: () => { this.saving.set(false); this.notify.success('Fees saved'); this.snapshotSection('fees'); },
      error: err => { this.saving.set(false); this.notify.error(err, 'Save failed'); },
    });
  }
  ...
  saveDocs(): void {
    ...
    this.api.saveServiceDocs(this.info().processCode, this.docs()).subscribe({
      next: () => { this.saving.set(false); this.notify.success('Documents saved'); this.snapshotSection('requiredDocs'); },
      error: err => { this.saving.set(false); this.notify.error(err, 'Save failed'); },
    });
  }
  ...
  saveDepts(): void {
    ...
    this.api.saveServiceDepts(this.info().processCode, this.depts()).subscribe({
      next: () => { this.saving.set(false); this.notify.success('Providers saved'); this.snapshotSection('relatedDepts'); },
      error: err => { this.saving.set(false); this.notify.error(err, 'Save failed'); },
    });
  }
  ...
  saveAudiences(): void {
    ...
    this.api.saveServiceAudience(this.info().processCode, this.audiences()).subscribe({
      next: () => { this.saving.set(false); this.notify.success('Target audience saved'); this.snapshotSection('targetAudiences'); },
      error: err => { this.saving.set(false); this.notify.error(err, 'Save failed'); },
    });
  }
  ...
  // ('' → null, but 0 and other falsy-yet-real values are kept)
  setInfo(field: keyof ProcessInfoDto, value: any): void {
    this.info.update(v => ({ ...v, [field]: value === '' || value === undefined ? null : value }));
  }
