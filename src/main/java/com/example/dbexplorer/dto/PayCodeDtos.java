...
public class PayCodeDtos {
    ...
    // ── Detail: LKP_PAY_CODE_DETAILS (one-to-one) ────────────────────────────

    public static class PayCodeDetailsDto {
        public Object id;                        // ID — auto-generated, hidden
        // FK columns (set from parent on save, not from UI):
    ...
    // ── Full payload (request + response) — one-to-one ───────────────────────

    public static class PayCodePayload {
        public PayCodeDto        payCode;
        public PayCodeDetailsDto details; // single record, null if none yet
    }
